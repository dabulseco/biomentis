"""
BiomentisEval1: Evaluation loader for Biomentis tasks

This class provides a unified interface to evaluate user answers against ground truth
for all tasks in the BiomentisEval1 benchmark.
"""
...
class BiomentisEval1:
    """
    Evaluation loader for BiomentisEval1 benchmark

    Usage:
        evaluator = BiomentisEval1('biomentis_eval1_dataset.parquet')
        score = evaluator.evaluate('gwas_causal_gene_opentargets', 0, 'BRCA1')
    """

    def __init__(self):
        """
        Initialize the BiomentisEval1 evaluator
        ...
        print(f"Loaded BiomentisEval1 dataset: {len(self.df)} instances across {self.df['task_name'].nunique()} tasks")
    ...
    def __repr__(self):
        return f"BiomentisEval1(instances={len(self.df)}, tasks={self.df['task_name'].nunique()})"
