...
try:
    ...
    from biomentis.config import default_config  # type: ignore
except Exception:
...
def search_protocols(
    ...
    if not ACCESS_TOKEN:
        raise ValueError(
            "Protocols.io access token is not configured. Set PROTOCOLS_IO_ACCESS_TOKEN or BIOMNI_PROTOCOLS_IO_ACCESS_TOKEN env var, or configure BiomentisConfig.protocols_io_access_token."
        )
...
def _get_protocols_directory():
    ...
    return os.path.join(os.path.dirname(biomentis.__file__), "tool", "protocols")
...
def read_local_protocol(filename: str, source: str | None = None) -> dict[str, Any]:
    """
    Read the contents of a local protocol file from biomentis/tool/protocols/.
