...
def api_schema_to_langchain_tool(api_schema, mode="generated_tool", module_name=None):
    if mode == "generated_tool":
        module = importlib.import_module("biomentis.tool.generated_tool." + api_schema["tool_name"] + ".api")
...
def read_module2api():
    ...
    module2api = {}
    for field in fields:
        module_name = f"biomentis.tool.tool_description.{field}"
        module = importlib.import_module(module_name)
        module2api[f"biomentis.tool.{field}"] = module.description
    return module2api
...
def parse_tool_calls_from_code(code: str, module2api: dict, custom_functions: dict = None) -> list[str]:
    ...
    Example:
        >>> code = "from biomentis.tool import analyze_data\nimport pandas as pd"
...
def inject_custom_functions_to_repl(custom_functions: dict):
    ...
    if custom_functions:
        ...
        from biomentis.tool.support_tools import _persistent_namespace
...
def format_detected_tools(detected_tool_modules: list) -> str:
    ...
    Example:
        >>> format_detected_tools([("analyze_data", "biomentis.tool"), ("pandas", "pandas")])
...
def remove_emojis_from_text(text: str) -> str:
    ...
    Note:
        The function targets specific emojis used in the Biomentis system:
        - 🔧 for tools
        - 📊 for data
        - ⚙️ for software
        - 📋 for configuration
...
def convert_markdown_to_pdf(markdown_path: str, pdf_path: str) -> None:
    ...
    try:
        ...
        <head>
            <meta charset="utf-8">
            <title>Biomentis Conversation History</title>
