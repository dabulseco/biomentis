# Biomentis Environment Setup

> **⚠️ Legacy directory — not the recommended install path anymore.**
> The current install is just three commands: `conda create -n biomentis python=3.11`, `pip install -r requirements.txt`, `pip install -e .`. See [`../quick_start_guide.md`](../quick_start_guide.md) at the repo root.
>
...
3. Lastly, to activate the biomentis environment:
```bash
...
```bash
conda activate biomentis_py310
```
