...
cleanup_old_installations() {
    ...
    local old_afs_dir="/afs/cs.stanford.edu/u/$(whoami)/biomentis_tools"
    ...
    local old_home_dir="$HOME/biomentis_tools"
...
echo "# Added by biomentis setup" >> "$TOOLS_DIR/setup_path.sh"
    ...
    if [ -n "$profile_file" ]; then
        ...
        if [ -w "$profile_file" ]; then
            ...
            if ! grep -q "export PATH=\"$TOOLS_DIR/bin:\$PATH\"" "$profile_file"; then
                # Remove any old biomentis_tools paths first
                if grep -q "biomentis_tools/bin" "$profile_file"; then
                    echo -e "${YELLOW}Removing old biomentis_tools paths from $profile_file...${NC}"
                    sed -i '/biomentis_tools\/bin/d' "$profile_file"
                fi

                echo "" >> "$profile_file"
                echo "# Added by biomentis setup" >> "$profile_file"
