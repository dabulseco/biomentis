...
DEFAULT_TOOLS_DIR="$(pwd)/biomentis_tools"
TOOLS_DIR=""
...
install_cli_tools() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "# Added by biomentis setup" >> setup_path.sh
...
main() {
    ...
    conda env create -n biomentis_e1 -f environment.yml
    ...
    if command -v micromamba &> /dev/null; then
        ...
        micromamba activate biomentis_e1
    else
        eval "$(conda shell.bash hook)"
        conda activate biomentis_e1
    fi
    handle_error $? "Failed to activate biomentis_e1 environment."
    ...
    echo -e "To activate this environment in the future, run: ${YELLOW}conda activate biomentis_e1${NC}"
    ...
    PATH=$(echo $PATH | tr ':' '\n' | grep -v "biomentis_tools/bin" | tr '\n' ':' | sed 's/:$//')
    export PATH="$(pwd)/biomentis_tools/bin:$PATH"
}
