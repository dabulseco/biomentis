# MCP (Model Context Protocol) Integration in Biomentis

Biomentis provides comprehensive support for the Model Context Protocol (MCP), allowing you to both integrate external MCP servers and expose Biomentis tools as MCP servers. This enables seamless interoperability with a wide ecosystem of AI tools and services.

## Overview

MCP (Model Context Protocol) is a standard protocol for AI applications to communicate with external tools and services. Biomentis supports two main MCP integration patterns:

1. **Adding External MCP Servers**: Import tools from external MCP servers into Biomentis
2. **Exposing Biomentis as MCP Server**: Make Biomentis tools available to other MCP clients

## Adding External MCP Servers to Biomentis
...
mcp_servers:
  ...
  pubmed:
    command: ["python", "-m", "biomentis.tool.mcp_tools.pubmed_mcp"]
    enabled: true
    tools:
      - biomentis_name: search_pubmed
      ...
      - biomentis_name: get_article_abstract
...
1. **Tool Discovery**: Biomentis automatically discovers available tools from each MCP server
2. **Async-to-Sync Wrapping**: MCP tools are wrapped to work with Biomentis's synchronous execution model
3. **Integration**: Tools are registered in Biomentis's tool registry and made available for retrieval
4. **Module Organization**: Each MCP server gets its own module namespace (e.g., `mcp_servers.github`)

### Tool Registration Process

When you call `add_mcp()`, Biomentis:
...
## Exposing Biomentis Tools as MCP Server

### Using the `create_mcp_server` Method

Biomentis can expose its internal tools as an MCP server, making them available to other MCP clients:
...
mcp = agent.create_mcp_server(tool_modules=[
    "biomentis.tool.genetics",
    "biomentis.tool.database",
    "biomentis.tool.cell_biology"
])
...
```bash
cd tutorials/examples/expose_biomentis_server
...
# Use GitHub tools alongside Biomentis tools
result = agent.go("""
Please list all repositories in the github account of the user.
Then use Biomentis's genetics tools to analyze any bioinformatics repositories found.
""")
```

### Exposing Biomentis as MCP Server
...
The Biomentis repository includes complete examples in the `tutorials/examples/` directory:
...
#### Exposing Biomentis as MCP Server
- **Location**: `tutorials/examples/expose_biomentis_server/`
- **Files**:
  - `run_mcp_server.py` - Script to run Biomentis MCP server
...
# Expose Biomentis as MCP server
cd tutorials/examples/expose_biomentis_server
...
You can test the Biomentis MCP server using the provided test script:

```python
#!/usr/bin/env python3
"""
Simple test script for testing a single Biomentis MCP tool.
"""
...
async def test_single_tool():
    """Test a single tool in the Biomentis MCP server."""
...
Biomentis's MCP integration provides a powerful way to extend its capabilities with external tools and services, while also making Biomentis tools available to the broader MCP ecosystem. This enables seamless interoperability and allows you to build sophisticated AI workflows that combine the best of multiple tool ecosystems.
